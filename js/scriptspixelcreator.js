alert("MADE BY LUCIAN AND ONLY LUCIAN");
// Default dimensions
...
function drawGrid(sw, sh, pw) {
  ...
  for (i = 0; i < sw / pw; i++) {
    $(".scene .col").append(
      `<div class="scene_pixelcolumn" style="width:${pw}px; height:${sh}px"></div>`
    );
  }

  for (i = 0; i < sh / pw; i++) {
    $(".scene .row").append(
      `<div class="scene_pixelrow" style="width:${sw}px; height:${pw}px"></div>`
    );
  }
}
...
$(".tools_grid").click(function () {
  if (grid) {
    ...
    $(".scene_pixelrow").css(
      "box-shadow",
      "0px 1px rgba(255,255,255, 0.2) inset, 1px 0px rgba(255,255,255, 0.2) inset"
    );
    $(".scene_pixelcolumn").css(
      "box-shadow",
      "0px 1px rgba(255,255,255, 0.2) inset, 1px 0px rgba(255,255,255, 0.2) inset"
    );
    grid = true;
...
function addPixelsToOutput() {
  ...
  if ($(".keyframes .frame").length == 1) {
    $(".scene_pixel").each(function () {
      output = `${output}${$(this).css("left").slice(0, -2) + "px"} ${
        $(this).css("top").slice(0, -2) + "px"
    ...
    for (i = 1; i < frameObject.length; i++) {
      if (frameObject[i] == "") {
        endAnimation.push(
          0 + i * (100 / frameObject.length) + "%{box-shadow: 0}"
        );
        endAnimation.push(
          0 + (i * (100 / frameObject.length) - 1) + "%{box-shadow: 0}"
        );
        endAnimation.push(
          0 + (i * (100 / frameObject.length) + 1) + "%{box-shadow: 0}"
        );
        ...
        endAnimation.push(
          0 +
            (i * (100 / frameObject.length) - 1) +
            "%{box-shadow: " +
            frameObject[i - 1].join() +
            "}"
        );
        endAnimation.push(
          0 +
    ...
    if (frameObject[frameObject.length - 1] == "") {
      ...
      endAnimation.push(
        99 + "%{box-shadow: " + frameObject[frameObject.length - 1].join() + "}"
      );
    }
...
function checkSymetry(el) {
  ...
  if (symY == true && symX == false) {
    ...
    $(".frame" + activeFrame + " .pixels").append(
      `<div class="scene_pixel" style="left:${relativex}px; top:${sym}px; width:${pw}px; position: absolute; height:${pw}px; background:${currentColor}"></div>`
    );
  }
  ...
  if (symX == true && symY == false) {
    sym = sw - (x + 1) * pw;
    $(".frame" + activeFrame + " .pixels").append(
      `<div class="scene_pixel" style="left:${sym}px; top:${relativey}px; width:${pw}px; position: absolute; height:${pw}px; background:${currentColor}"></div>`
    );
  }
  ...
  if (symX == true && symY == true) {
    ...
    symA = sh - (y + 1) * pw;
    $(".frame" + activeFrame + " .pixels").append(
      `<div class="scene_pixel" style="left:${relativex}px; top:${symA}px; width:${pw}px; position: absolute; height:${pw}px; background:${currentColor}"></div>`
    );
    $(".frame" + activeFrame + " .pixels").append(
      `<div class="scene_pixel" style="left:${sym}px; top:${relativey}px; width:${pw}px; position: absolute; height:${pw}px; background:${currentColor}"></div>`
    );
    ...
    $(".frame" + activeFrame + " .pixels").append(
      `<div class="scene_pixel" style="left:${sym}px; top:${symA}px; width:${pw}px; position: absolute; height:${pw}px; background:${currentColor}"></div>`
    );
  }
}
...
function draw(e, el) {
  ...
  if (x < sw / pw && y < sh / pw && x > -1 && y > -1) {
    ...
    $(".frame" + activeFrame + " .pixels").append(
      `<div class="scene_pixel" style="left:${relativex}px; top:${relativey}px; width:${pw}px; position: absolute; height:${pw}px; background:${currentColor}"></div>`
    );
...
function deletePixel(e, el) {
  ...
  $(".frame" + activeFrame + " .scene_pixel").each(function () {
    if (
      $(this).css("top") == `${y * defPixelWidth}px` &&
      $(this).css("left") == `${x * defPixelWidth}px`
    ) {
      $(this).remove();
    }
  });
...
const form = `<form action="https://codepen.io/pen/define" method="POST" target="_blank"><input class="change" type="hidden" name="data" value='${JSONstring}'><input type="submit" width="40" height="40" value="Export to new pen"></form>`;
...
function makeCanvas(f) {
  ...
  let data = `<svg xmlns="http://www.w3.org/2000/svg" style="background:${bgColor};" width="1000" height="1000"><foreignObject width="100%" height="100%"><div xmlns="http://www.w3.org/1999/xhtml" style="font-size:40px">${$(
    ".frame" + f + " .pixels"
...
$(".keyframes .frames .frame1").append(
...
$(".fa-copy").click(function () {
  ...
  $(".scene").append(
    '<div class="frame frame' +
      frames +
      '"><div class="pixels">' +
      $(".scene .frame" + $(this).parent().data("frame")).html() +
      "</div></div>"
  );

  $(".activeF").removeClass("activeF");
  $(".keyframes .frames .wrap").append(
    '<div data-frame="' +
      frames +
      '" class="frame activeF frame' +
      frames +
      '"><div class="frameTitle">Frame #' +
      frames +
      '</div><i class="fa fa-trash"> </i><i class="fa fa-copy"> </i></div>'
  );
  $(".keyframes .frames .frame" + frames).append(
    '<canvas id="canvas' + frames + '"></canvas>'
  );
  activeFrame = frames;
  onionSkin(frames);
  buildKeyframes();
...
$(".keyframes .add").click(function () {
  ...
  $(".scene .frame").hide();
  $(".scene").append(
    '<div class="frame frame' + frames + '"><div class="pixels"></div></div>'
  );
  $(".activeF").removeClass("activeF");
  $(".keyframes .frames .wrap").append(
    '<div data-frame="' +
